...
export const likeUnlikeComment = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const comment = await feedCommentService.findById(commentId, authenticatedUser.id, true);
        if (!comment) {
            await transaction.rollback();
            return sendNotFoundResponse(res, responseMessages.feedCommented.notFound);
        }

        const isLiked = await commentLikeService.checkIfLiked(commentId, authenticatedUser.id);

        if (isLiked) {
            const unliked = await commentLikeService.removeCommentLike(commentId, authenticatedUser.id, authenticatedUser.id, transaction);
            if (!unliked) {
                await transaction.rollback();
            ...
            try {
                await emitFeedCommentUpdated(commentId, comment.feed_id, feedCommentService, CommentLike);
        ...
        const liked = await commentLikeService.createCommentLike(commentId, authenticatedUser.id, authenticatedUser.id, transaction);
        ...
        if (commentAuthorId && commentAuthorId !== authenticatedUser.id) {
            ...
            await notificationService.sendCommentLikedNotification(commentAuthorId, authenticatedUser.id, likerName, commentId, comment.feed_id, transaction, commentContent);
        }
        await transaction.commit();
        transactionCommitted = true;
...
export const getCommentLikes = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const comment = await feedCommentService.findById(commentId);
        if (!comment) {
            return sendNotFoundResponse(res, responseMessages.feedCommented.notFound);
        }

        const likes = await commentLikeService.findByCommentId(commentId);
