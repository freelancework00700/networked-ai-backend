...
import { isEmailEnabled } from "../services/service-settings.service";
...
export const sendEmail = async (mailOptions: MailOptions): Promise<any> => {
    try {
        // Check if Email service is enabled
        const emailEnabled = await isEmailEnabled();
        if (!emailEnabled) {
            loggerService.info(`Email service is disabled. Skipping email`);
            return null;
        }
        ...
        loggerService.error(`Error checking email service status: ${error.message}`);
        throw error;
    }
};
